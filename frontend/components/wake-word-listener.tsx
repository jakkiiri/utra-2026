...
interface WakeWordListenerProps {
  ...
  onStartDictation?: () => void  // Trigger manual dictation to take over
  enabled?: boolean
  ...
  useEffect(() => {
    ...
    recognitionRef.current.onresult = (event) => {
      ...
      if (containsWakeWord(transcript)) {
        console.log('🎤 Wake word "Winter" detected, handing over to dictation system')
        ...
        if (!wakeWordActive) {
          ...
          // Notify parent - triggers UI changes
          onWakeWordDetected()

          // Hand over control to manual dictation system
          if (onStartDictation) {
            console.log('👉 Starting manual dictation to capture question')
            setTimeout(() => {
              onStartDictation()
            }, 300) // Small delay to ensure wake word recognition fully stops
          }
  ...
  return (
    <div className="fixed bottom-4 right-4 z-40">
      <div
        ...
        title={isListening ? "Say 'Hey Winter' to ask a question" : "Wake word listener paused"}
      >
        <Mic className="size-3" />
        <span className="hidden sm:inline">
          {isListening ? 'Say "Hey Winter"' : 'Paused'}
        </span>
      </div>
    </div>
  )
